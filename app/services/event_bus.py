...
class EventBus:
    ...
    def on(self, event_type: str, *, module: str = "unknown") -> Callable:
        ...
        def decorator(fn: Callable) -> Callable:
            ...
            logger.debug("event_subscribed", event_name=event_type, module=module)
            return fn
        return decorator
    ...
    async def emit(
        ...
        *,
        ...
        if not listeners:
            logger.debug("event_no_listeners", event_name=event_type)
            return event
        ...
        for (_, mod), result in zip(listeners, results):
            if isinstance(result, Exception):
                logger.error("event_listener_error", module=mod, event_name=event_type, error=str(result))
            else:
